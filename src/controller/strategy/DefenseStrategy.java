...
/**
 * A defensive strategy for the Three Trios game. Focuses on minimizing the number of
 * exposed sides when placing a card.
 */
public class DefenseStrategy extends StrategyAbstract {


  /**
   * Evaluates how effective a move is defensively. Considers the number of exposed sides
   * and the attack values of those sides.
  ...
  public int effectiveness(Move move, Supplier<Model> modelSupplier) {
    ...
    List<CardinalDirection> exposedSides = getExposedSides(move.row, move.col, cell);
    return ((4 - exposedSides.size()) * 10) + exposedSides.stream()
                                                          .map(
                                                              (cd) -> card.getAttackValue(
                                                                  cd).ordinal())
                                                          .reduce(0, Integer :: sum);
  }

  // card cell and is empty
  protected List<CardinalDirection> getExposedSides(int row,
                                                    int col,
                                                    GridCellReadOnly cell) {
    return Arrays.stream(CardinalDirection.values())
                 .filter((cd) -> cell.hasNeighborToThe(cd) && cell.getNeighborToThe(cd)
                                                                  .canHaveCard()
